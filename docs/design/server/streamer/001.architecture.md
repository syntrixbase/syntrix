...
│                       │                                            │
│  ┌────────────────────▼─────────────────────────────────────────┐ │
│  │           JetStream Event Consumer (NEW)                      │ │
│  │  ┌────────────┐  ┌──────────────┐  ┌──────────────────────┐ │ │
│  │  │ JetStream  │  │ Deduplicator │  │   Event Parser       │ │ │
│  │  │ Subscriber │  │              │  │                      │ │ │
│  │  │            │  │ (3min cache) │  │                      │ │ │
│  │  └────────────┘  └──────────────┘  └──────────────────────┘ │ │
│  └────────────────────┬─────────────────────────────────────────┘ │
...
- **Removed Direct MongoDB Connection**: Streamer now consumes from Puller via JetStream
...
### 3.3 JetStream Event Consumer

**Purpose:** Consume normalized events from Puller service via NATS JetStream.

**Data Flow:**

```go
NATS JetStream (from Puller) → Consumer → Deduplicator → Matcher
```
...
type JetStreamEventConsumer struct {
    ...
    // Subscriptions per collection/partition
    subs map[string]*nats.Subscription
}

func (c *JetStreamEventConsumer) Start(ctx context.Context) error {
    // Subscribe to subjects: puller.events.{collection}.{partition}
    // Note: Puller handles MongoDB connection, partitioning, and normalization

    for _, subject := range c.getSubscribeSubjects() {
        sub, err := c.js.Subscribe(subject, c.handleMessage,
            nats.ManualAck(),
            nats.AckExplicit(),
            nats.DeliverAll(),
...
**Multi-Backend and Partition Support:**

Streamer subscribes to subjects published by Puller:

```
Subject Pattern: puller.events.{collection}.{partition}
or for multi-backend: puller.events.{backend}.{collection}.{partition}

Examples:
- puller.events.users.0          (single backend, partition 0)
- puller.events.users.1          (single backend, partition 1)
- puller.events.default.users.0  (multi-backend mode)
- puller.events.vip-a.users.0    (dedicated backend for VIP tenant)
```

**Subscription Strategy:**

```go
func (c *JetStreamEventConsumer) getSubscribeSubjects() []string {
    // Option 1: Subscribe to all collections (wildcard)
    return []string{"puller.events.>"}

    // Option 2: Subscribe to specific collections
    // return []string{
    //     "puller.events.users.>",
    //     "puller.events.orders.>",
    // }
}
```

**Benefits of JetStream Integration:**

1. **No Direct MongoDB Connection**: Streamer doesn't need MongoDB credentials or connection management
2. **Automatic Load Balancing**: Multiple Streamer instances can consume from the same stream
3. **Built-in Persistence**: JetStream stores events, providing replay capability
4. **Backpressure Handling**: NATS handles flow control automatically
...
func (d *Deduplicator) IsDuplicate(evt *NormalizedEvent) bool {
    // Note: Puller already normalizes events, but deduplication is still
    // needed in case of JetStream redelivery or multiple Streamer instances
...
The Matching Engine is where CEL filter expressions are evaluated. This design reduces NATS traffic by ~90% compared to pushing all events to Gateway and filtering there.
...
|--------|---------------|-----------------|
| NATS Traffic | 100% (all events) | ~10% (only matches) |
...
func (s *Streamer) Start(ctx context.Context) error {
    ...
    // 3. Start JetStream consumer (subscribes to Puller events)
    if err := s.jetStreamConsumer.Start(ctx); err != nil {
...
streamer:
  ...
  nats:
    ...
    consumer_name: "streamer-consumer"

    # Input: Subscribe to Puller events
    input_subjects:
      - "puller.events.>"  # Subscribe to all collections

    # Output: Publish to Gateway
...
|---------|-----------|----------|
| NATS connection lost | Consumer error | Automatic reconnection by NATS client |
| JetStream consumer lag | Monitoring metrics | Horizontal scaling of Streamer instances |
...
### 7.3 JetStream Specific Error Handling
...
func (c *JetStreamEventConsumer) handleMessage(msg *nats.Msg) {
    defer func() {
        if r := recover(); r != nil {
            ...
            // Negative acknowledgment - JetStream will redeliver
            msg.Nak()
        }
    }()
    ...
    if err := c.processEvent(msg); err != nil {
        ...
        if metadata.NumDelivered > 3 {
            log.Error("Max retries exceeded, terminating message")
            msg.Term()  // Remove from stream
        } else {
            msg.NakWithDelay(time.Second * 5)  // Retry after delay
        }
        return
    }
