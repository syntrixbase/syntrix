...
| gRPC Server | `internal/query/grpc/server.go` | New |
| gRPC Client | `internal/query/client/client.go` | Replace |
| Conversion | `internal/query/grpc/convert.go` | New |
| Interface | `internal/query/interface.go` | Add factory |
| Manager | `internal/services/manager_init.go` | Update init |
| REST API | `internal/gateway/rest/handler.go` | Update import |
| Realtime | `internal/gateway/realtime/server.go` | Update import |
| Authz | `internal/core/identity/authz/engine.go` | Update import |
