...
5. **Trigger Delivery Service**: Dedicated module for triggers and outbound webhooks.
   ...
   - **Evaluator**: Evaluates trigger conditions using CEL expressions (`internal/trigger/evaluator/`).
   - **NATS Queue**: Decouples evaluation from delivery for reliability (`internal/trigger/pubsub/`).
   - **Worker**: Consumes tasks from NATS, delivers webhooks with exponential backoff retry (`internal/trigger/worker/`).
...
1. **Gateway (`syntrix --gateway`)**: Unified REST/gRPC/WS/SSE entry; handles auth, CRUD routing, replication, and realtime delivery.
   - Code: `internal/gateway/rest/`, `internal/gateway/realtime/`
...
- Code layout under `internal/gateway/`:
...
Gateway (REST Handler)
    │ internal/gateway/rest/handler_document.go
    ▼
Query Engine
    │ internal/query/core/engine.go
    ▼
Storage Layer
    │ internal/core/storage/mongo/document_store.go
    ▼
MongoDB
```
...
Puller (Listen & Buffer)
    │ internal/puller/core/
    │ Buffer: PebbleDB
    ▼
Streamer (Match & Route)
    │ internal/streamer/manager/
    │ Evaluator: CEL expressions
    ▼
Gateway (Realtime)
    │ internal/gateway/realtime/hub.go
    ▼
Client (WS/SSE)
```
...
Streamer (matched event)
    ...
    ▼
Trigger Evaluator
    │ internal/trigger/evaluator/
    │ Condition: CEL expressions
    ▼
NATS Publisher
    │ internal/trigger/pubsub/
    ▼
Trigger Worker
...
2) Directory structure: ensure `internal/gateway/rest` and `internal/gateway/realtime` exist under `internal/gateway`; migrate existing code accordingly.
3) Server assembly: in `internal/gateway/server.go`, register REST/replication routes and `/realtime/ws|sse` on one mux/server.
...
The codebase is organized under `internal/` with the following structure:

### 7.1 Core Domain (`internal/core/`)

Core domain abstractions shared across all services:

```text
internal/core/
├── identity/           # Authentication & Authorization
│   ├── authn/          # Authentication service (JWT, tokens)
│   ├── authz/          # Authorization rules engine (CEL-based)
│   ├── config/         # Identity configuration
│   └── types/          # Shared identity types
└── storage/            # Data Persistence Layer
    ├── config/         # Storage configuration
    ├── mongo/          # MongoDB implementation
    ├── router/         # Multi-backend routing
    └── types/          # Storage types and interfaces
```

### 7.2 Gateway (`internal/gateway/`)

Unified API entry point for REST, Realtime, and Replication:
...
└── server.go           # Unified HTTP server
```

### 7.3 Service Packages

Each service package follows a flat structure:
...
internal/puller/
├── config/             # Puller configuration
├── core/               # Change stream listener
├── buffer/             # PebbleDB event buffer
├── events/             # Event types
├── grpc/               # gRPC server
├── client/             # gRPC client
└── service.go          # Service interface
...
```text
internal/services/      # Service Manager (orchestration)
├── manager.go          # Service lifecycle management
├── manager_init.go     # Service initialization
└── manager_start.go    # Service startup

internal/server/        # Unified HTTP/gRPC Server
