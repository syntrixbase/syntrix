...
Index Manager
    ...
    └─> map[string]*Database  (keyed by database name)
            ...
            └─> Database
                  ...
                  ├── Mu: RWMutex
                  │
                  └─> map[string]*Shard  (keyed by pattern|templateIdentity)
                          │
                          └─> Shard
                                ├── Pattern: "users/*/chats"
...
- Second layer: `map[pattern|template]*Shard` for index shards within each database

**Benefits**:
- Database deletion: remove entire Database object without scanning all shards
- Per-database statistics and configuration
- Clearer code structure aligned with business concepts
- Same collection pattern in different databases = completely isolated shards
