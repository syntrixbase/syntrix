...
Index templates define which fields to index for which collections.
...
- Path segments only; `{var}` matches one segment
- Variable names are ignored for matching (normalize to `*`)
- Indexes are collection-scoped (pattern must end at collection root)
- Priority: more fixed segments > fewer placeholders > longer path
...
Index Manager
    │
    └─> map[string]*Shard  (keyed by normalizedPattern|templateIdentity)
            │
            └─> Shard
                  ├── Pattern: "users/*/chats"
...
```
[version:1][field1...][field2...][id_len:uvarint][id bytes]

- string (asc): [len:uvarint][utf8 bytes]
- string (desc): invert each byte after length
- number: big-endian sortable bytes; desc invert
