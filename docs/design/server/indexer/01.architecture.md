...
## Public Surface
- Interface: `Service` (existing contract) with explicit `database string` on every call; empty database resolves to `model.DefaultDatabaseID` for backward compatibility only.
...
### Database Handling
- Edge validation: HTTP/replication/watch request bodies include `database`; reject blank database in prod, allow default database for dev/tests.
- Isolation: Data/Index/Query store and look up documents, indexes, checkpoints, and change events scoped by database. No cross-database reads or writes.
- Index partitioning: index namespaces and rebuild state are per-database-per-collection; change-stream checkpoints include database to avoid leakage during resume.

## Internal Package Layout (proposed)
- `internal/indexer/internal/data`: data adapters over `storage.DocumentStore`; pull change events from puller for index.
- `internal/indexer/internal/index`: index adapters/manager; supports upsert/delete, search (prefix/range), rebuild.

### Module Details
- Data
        - Responsibilities: CAS-aware CRUD/Patch/Delete on storage; flatten/strip protected fields; prefix scan iterators; change stream emission (docid, collection, version, deleted, timestamps, payload hash/payload*); backpressure to protect storage during rebuild scans.
        - Interfaces: `Get/BatchGet/ScanPrefix`.
...
- Event fields (minimum): `database`, `docid`, `collection`, `version`, `deleted`, `updatedAt`, `createdAt`, `dataHash` (or payload fingerprint), optional `payload` for replay.
...
- Reads: Service → Query → Index (candidate IDs + order) → Data batch fetch → assemble documents; fallback to Data prefix scan if index unavailable. All lookups scoped by database.
- Watch: Service → Query → Data/Index stream → HTTP streaming (SSE/long poll) unchanged on the wire; streams are database-isolated.
- Replication: Service → Query/Data for pull; push uses Data CAS semantics; checkpoints, change tokens, and deleted flags include database.
