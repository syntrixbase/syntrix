...
- Per-database: `map[string]*Shard` keyed by `(normalizedPattern|templateIdentity)` to allow multiple indexes per collectionPattern.

**Database fields**:
  - `Name string` (e.g., `myapp`).
  - `Mu` (RW lock) guarding the database.
  - `shards map[string]*Shard` - index shards within this database.
...
- Concurrency: per-database and per-shard RW locks; applies batch events under write lock; readers take read lock (or snapshot view if backend supports).
...
| shards:          |                          | shards:          |
|   └─> map[key]*Shard                        |   └─> map[key]*Shard
+------------------+                          +------------------+
        |                                               |
  (pattern|templateIdentity)                    (pattern|templateIdentity)
        |                                               |
+--------------------------------------+    +--------------------------------------+
| Shard: users/*/chats|name:asc,age:desc|  | Shard: users/*/chats|name:asc,age:desc|
+--------------------------------------+    +--------------------------------------+
...
+--------------------------------------+    +--------------------------------------+
         ↑ Same pattern, different databases = completely isolated shards
