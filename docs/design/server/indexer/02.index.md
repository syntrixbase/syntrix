...
- Encoding: key uses fields in order; numeric big-endian, strings length-prefixed; may append `deleted`/`version` for tie-break; cursor/startAfter share the same encoding with a version byte (OrderKey encoding versioning). A version bump means the same logical index is rebuilt with a new encoding; it is **not** a new template/index definition.
 - Encoding (OrderKey):
   ...
   - For each indexed field (in order):
     ...
     - string: 2-byte big-endian length (fixed width) + raw bytes (UTF-8); asc as-is; **desc invert ALL bytes (including type tag and length)** to ensure correct lexicographic order.
...
string field (asc): [0x03][len:2B big-endian][utf8 bytes]
string field (desc): invert ALL bytes (including type tag and length)
...
**Why fixed-width length for desc?** Using uvarint length with only content bytes inverted would cause "abc" (len=3) to sort before "ab" (len=2) in descending order, because the length prefix still sorts ascending. By using fixed 2-byte length and inverting all bytes together, we ensure correct descending sort.
...
- strings: [0x03][len:2B big-endian][utf8 bytes]; desc invert ALL bytes
...
function search(database, plan):
  ...
  // Step 2: Derive shard key from template
  ...
  // Step 3: Lookup shard
  shard = lookupShard(database, pattern, templateIdentity)
