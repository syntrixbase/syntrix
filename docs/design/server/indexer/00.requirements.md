...
- Honor multi-database isolation across the stack (service, HTTP, query, replication, watch) with explicit database parameters and safe defaults.

## Functional Requirements (How)
- Public API: expose `Service`, `NewService(storage.DocumentStore, pullerURL string) Service`, `NewClient(baseURL string) Service`, and `NewHTTPHandler(s Service) http.Handler`; no other exported structs.
- Multi-database surface: every `Service` method accepts an explicit `database string`; HTTP/replication/watch requests carry `database` in the body; empty databases resolve to `model.DefaultDatabaseID` only for backward compatibility in dev/test.
- Database isolation: Data, Index, Query, Watch, and Replication must not read/write across databases; change events, checkpoints, and index partitions include the database id.
...
- Validate database input on the edge; default database fallback is temporary and should be removable once callers send explicit database.
- Avoid new external dependencies unless approved; reuse testify for tests.
- Follow package visibility rules: implementations reside under `internal/indexer/internal/...`.
