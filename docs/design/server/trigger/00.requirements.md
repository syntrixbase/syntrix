...
- Support service separation: Evaluator and Delivery services can run independently.
...
- Checkpoints: **Resolved** - Uses global checkpoint key `sys/checkpoints/trigger_evaluator` because Puller returns aggregated progress tokens. See [evaluator/01.checkpoint.md](evaluator/01.checkpoint.md).
