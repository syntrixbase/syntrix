...
- Enforce database isolation via subject prefixing on publish/consume and database-scoped checkpoints so resume tokens do not leak across databases.
...
- Secrets/tokens are database-scoped; cross-database reuse is forbidden.
- Shared `TRIGGERS` stream is assumed; monitor per-database traffic/retention and be ready to pivot to per-database streams if quotas/noisy-neighbor risks emerge.

## Observability baseline

- Emit per-database metrics for publish/consume/worker success, retry counts, and latency; tracing is optional but recommended for incident drills.
...
- Metrics shape: at minimum expose counters for publish/consume/worker success and retries, and histograms for end-to-end latency, all tagged with database and collection.
- Subject-length guard details: use NATS subject max of 1024 bytes; if `triggers.<database>.<collection>.<encodedDocKey>` exceeds this, replace `encodedDocKey` with `sha256(<encodedDocKey>)` hex truncated to 32 chars; always carry the original decoded docKey in the payload.
...
- Multi-database isolation: decided to keep a single `TRIGGERS` stream with subject prefixing (`triggers.<database>.>`) for routing; revisit only if quotas/retention require per-database streams.
- Checkpoints: must be database-scoped keys (e.g., `sys/checkpoints/trigger_evaluator/<database>`) to avoid cross-database resume token sharing.
