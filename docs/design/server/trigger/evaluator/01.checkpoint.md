...
┌─────────────────┐
│ Trigger Watcher │ ──► Filters by DatabaseID field
└─────────────────┘
```

### 2. DatabaseID is Logical, Not Physical

The `DatabaseID` field in events refers to the **Syntrix logical database** (stored in document metadata), NOT the MongoDB database instance.
...
type StoredDoc struct {
    ...
    DatabaseID string                 // Syntrix logical database (NOT MongoDB database)
    Collection string
...
func (w *pullerWatcher) Watch(ctx context.Context) (<-chan events.SyntrixChangeEvent, error) {
    ...
    go func() {
        for pEvent := range pullerCh {
            ...
            if w.database != "" && pEvent.Change.DatabaseID != w.database {
