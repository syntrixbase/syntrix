...
6) Checkpoint updated after each processed event using a global key (at-least-once semantics). Note: Puller returns a single aggregated progress token across all databases, so checkpoint is global. The `Database` in events refers to the Syntrix logical database, not MongoDB instance.
