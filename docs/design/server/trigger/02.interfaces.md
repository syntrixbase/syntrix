...
- `internal/trigger/watcher`: storage watch + checkpoint adapter.
- `internal/trigger/pubsub`: NATS publisher/consumer + config.
- `internal/trigger/worker`: HTTP worker, auth/signature helpers.
- `internal/trigger/config`: trigger config structs (NATS, HTTP, retry/backoff, pools).
