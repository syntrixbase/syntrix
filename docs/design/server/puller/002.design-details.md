...
Checkpoint tokens are persisted in the per-backend PebbleDB buffer and are written in the same batch as events for atomicity. Recovery reads the latest token from the local buffer on startup.
...
```text
Checkpoint key: "!checkpoint/resume_token"
...
3. `Set(checkpointKey, token)` (Always update with the latest token in the batch)
4. `Commit(pebble.Sync)`

**Shutdown flow:**

On graceful shutdown, the buffer flushes any pending writes, ensuring the latest checkpoint is saved.

**Notes:**

- Each backend already has its own Pebble buffer directory, so checkpoint keys remain isolated per backend.
- Checkpoints are always as up-to-date as the durable events.
