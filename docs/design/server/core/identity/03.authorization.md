...
Security rules are configured per-database using a directory-based structure. Each database has its own rules file.
...
- `request.auth.userId`: The authenticated user's ID.
- `request.auth.username`: The authenticated user's username.
- `request.auth.roles`: The roles assigned to the user.
...
### 5.3 Rule validation & limits (publish-time)
