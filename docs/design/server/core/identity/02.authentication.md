...
- Single-database, self-hosted focus; external IdPs out of scope for this iteration.
...
- Non-goals (now): multi-factor auth, social login, SSO, SCIM, org/database hierarchy, password reset via email/SMS. Admin UI is documented separately in [010_console.md](010_console.md).
...
- **Claims (minimal)**: `sub` (user id), `username`, `roles` (optional), `db_admin` (database admin list), `iat`, `exp`, `jti`, `disabled` flag.
...
type Claims struct {
    ...
    Roles    []string `json:"roles"`    // optional role list
    DBAdmin  []string `json:"db_admin"` // databases with admin access (bypass authz)
    IAT      int64    `json:"iat"`      // issued at
    EXP      int64    `json:"exp"`      // expiration
    JTI      string   `json:"jti"`      // JWT ID
    Disabled bool     `json:"disabled"` // account disabled flag
}
```

**`db_admin` field:**
- Lists databases where the user has full admin access
- Users with a database in `db_admin` bypass authz rules for that database
- Empty array means the user must pass authz rules for all databases

### 4.2 Default parameters (proposed)
...
- Introduce database/project model; scope rules and keys per database.
...
- Database/project model and per-database key/rule isolation.
