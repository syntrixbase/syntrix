...
> **Related:** See [gateway/06.api-url-structure.md](../../gateway/06.api-url-structure.md) for the API URL structure with explicit database paths.

## 2. Requirements
- No implicit default database; all API requests must include database in URL path.
...
- How: add `database` to data model and APIs; enforce filters at store/query layer; extend factory/router to map database→backend.
...
## 6. API & Types Changes
- All API endpoints include database in URL path: `/api/v1/databases/{database}/documents/{path}`, `/api/v1/databases/{database}/query`, etc.
...
- Token no longer contains `database` claim; database is extracted from URL path. See [identity/02.authentication.md](../identity/02.authentication.md) for `db_admin` claim.
