...
- Default isolation: shared collections with strong row-level segregation via `database_id` (top-level field in Mongo documents).
...
- Mongo stores filter by `_id`, `collection`, `deleted`; no `database_id` filter/index.
...
- How: add `database_id` to data model and APIs; enforce filters at store/query layer; extend factory/router to map database→backend.

## 5. Data Model Changes (Mongo)
- Add top-level `database_id` to all collections (documents, users, revocations).
- ID strategy: `_id` is database-aware using prefix format, e.g. `_id = database_id + ":" + hash(fullpath)` (and similarly for users/revocations). This guarantees global uniqueness and allows regex-based filtering in Change Streams (Watch) for delete events.
- Indexes:
  - Documents: `_id` as primary; store `collection_hash` (using existing hash implementation) and index `(database_id, collection_hash)` for scans; keep TTL on `sys_expires_at` scoped by database; retain the plain `collection` field for readability/debug.
  - Users: unique `(database_id, username)`; `_id` generated with database namespace (e.g. `database_id + ":" + hash(user_id_source)`) to avoid cross-database collisions; `(database_id, username)` enforces per-database uniqueness.
  - Revocations: `_id` database-aware; TTL on `expires_at`; mandatory `(database_id, expires_at)` to support per-database sweeping.
...
- ID helpers become database-aware (e.g. `CalculateDatabaseID(database, fullpath)`); path-only helpers are deprecated to avoid cross-database collisions.
- Context objects (if introduced later) must still carry explicit database and be validated before store calls.

## 7. Routing & Factory
- Factory maintains map: `database_id -> backend binding`. Defaults to shared backend; config can mark databases as dedicated.
...
- CRUD/Query/Watch filters always include `database_id`.
- Soft delete and TTL updates set `database_id` in writes; queries exclude deleted by default and filter database.
- Watch: server-side pipeline filters by database using `database_id` field for inserts/updates, and `documentKey._id` prefix match (`^database_id:`) for deletes.
- EnsureIndexes creates compound indexes including `database_id`.
...
- Backfill path: if needed later, set `database_id` = `default` for legacy docs.
...
- Index verification tests: `(database_id, collection_hash)` exists; TTL indexes remain; `_id` generation is database-aware (same path with different database yields different `_id`).
...
- Implement Mongo filters/indexes with `database_id`.
- Add tests per plan.
