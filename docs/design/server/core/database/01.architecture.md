...
### 3.3 Database ID Generation

Database IDs are auto-generated using:
```
id = hex(blake3(uuid())[:8])
```

**Properties:**
- Length: 16 characters (first 8 bytes of blake3 hash as hex)
- Format: lowercase hexadecimal
- Example: `a1b2c3d4e5f67890`
- Collision-resistant: UUID ensures uniqueness, blake3 provides compact representation
...
**Reserved IDs:**
- `default` - System default database (special case, not auto-generated)
...
4. `EnsureDefaultDatabase()` - Create `default` database owned by `syntrix`

**Note:** The `default` database is owned by the `syntrix` admin user (created on system startup).
...
A user can be a `db_admin` for a database without owning it, and vice versa.
...
| Action | Permission |
|--------|------------|
| Use database (CRUD) | Database exists AND status is active AND authorized by security rules |
