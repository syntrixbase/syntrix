...
Using ID:
```http
PATCH /admin/databases/id:a1b2c3d4e5f67890
Content-Type: application/json
Authorization: Bearer <admin-token>
...
```http
GET /api/v1/databases/my-app-prod/documents/users/123
```
...
```json
{
    "error": {
        ...
        "message": "Database 'my-app-prod' is suspended"
    }
}
```
...
- Database metadata (including slug)

**Warning:** This operation is irreversible.

### 4.2 Slug Recycling

When a database is deleted (hard delete after cleanup):
- The slug is **immediately released** for reuse
- Another user can create a database with the same slug right away
- This enables quick recovery scenarios (delete and recreate with same slug)

**Note:** During the `deleting` status (soft delete), the slug is still reserved.
...
Using slug:
```http
DELETE /api/v1/databases/my-app-prod
Authorization: Bearer <user-token>
```

Using ID:
```http
DELETE /api/v1/databases/id:a1b2c3d4e5f67890
Authorization: Bearer <user-token>
```

**Admin API**
```http
DELETE /admin/databases/my-app-prod
Authorization: Bearer <admin-token>
```
...
### 4.5 Impact on Document Operations
...
Using ID:
```http
GET /api/v1/databases/id:a1b2c3d4e5f67890/documents/users/123
```
...
|----------|------|--------|
| Default | `default` | System default database, always required |

```go
// CheckProtectedDatabase checks if a database is protected from deletion.
// Uses slug (not ID) to identify protected databases.
func (s *Service) CheckProtectedDatabase(db *Database) error {
    if db.Slug != nil && *db.Slug == "default" {
...
func (s *Service) DeleteDatabase(ctx context.Context, userID string, isAdmin bool, identifier string) error {
    ...
    db, err := s.ResolveDatabase(ctx, identifier)
    if err != nil {
        return err
    }

    // Check protected databases (by slug)
    if err := s.CheckProtectedDatabase(db); err != nil {
        return err
    }
...
**Important:** Databases in `deleting` status **still count toward user quota** until the background worker completes hard deletion.

This prevents abuse via rapid create/delete cycles:
1. User creates database (quota: 1/3)
2. User deletes database (status = `deleting`, quota still: 1/3)
3. User creates another database (quota: 2/3)
4. Background worker hard-deletes (quota: 1/3)

```go
// CountDatabasesByOwner counts ALL databases including 'deleting' status
