...
- Database is not part of `model.Query`; it is carried separately on every `Service` call and HTTP request body. Empty database resolves to `model.DefaultDatabaseID` for dev/test only.
...
- Enforce database scoping: plan, fetch, watch, and replication ops must never cross databases; all cursors and checkpoints are database-scoped.
