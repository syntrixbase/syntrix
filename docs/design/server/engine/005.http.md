...
- Enforce database on edge: read `database` from the body, apply default database only in dev/test, and reject empty database in prod paths once callers are updated.
...
- Multi-database request shape: each POST body already includes `database`; keep response bodies database-neutral. Consider future header-based auth-derived database to reduce payload duplication.
