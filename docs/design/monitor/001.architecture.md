...
- **Instrumentation (How)**: OpenTelemetry SDKs in Go services and JS/TS SDK; standard attributes: `service`, `version`, `database`, `region`, `request_id`, `trace_id`, `plan_id` (queries), `user_id` when allowed.
...
- **Traces path (How)**: OTLP -> tail-sampling -> Tempo/Jaeger; store traces 7-14 days; index `database`, `error`, and `plan_id` fields for search.
...
- Mandatory `database` label; collectors reject or rewrite unlabeled data.
