...
- **Instrumentation (How)**: OpenTelemetry SDKs in Go services and JS/TS SDK; standard attributes: `service`, `version`, `database_id`, `region`, `request_id`, `trace_id`, `plan_id` (queries), `user_id` when allowed.
...
- **Collectors (How)**: HA pair per region; batch processor to reduce export chatter; attribute processor to enforce required labels and drop high-cardinality fields; tail-sampling for error and high-latency traces; rate limits per database to protect pipeline.
...
- **Traces path (How)**: OTLP -> tail-sampling -> Tempo/Jaeger; store traces 7-14 days; index `database_id`, `error`, and `plan_id` fields for search.
...
- **Alerting (How/Why)**: Alertmanager receives Prometheus alert rules; paging (P1/P2) for SLO burn-rate breaches; notification templates include runbook link, recent deploy, top offenders (database/endpoint), and trace exemplars.
- **Dashboards (How)**: Grafana with service owner views (RED/USE + dependencies), database slices, and executive rollups; deployment markers from CI/CD for correlation.
...
- Mandatory `database_id` label; collectors reject or rewrite unlabeled data.
- Per-database sampling and log volume caps; per-database dashboards and SLO views.
...
- Phase 3: Harden alerting (burn-rate), add runbooks, enable per-database caps; production rollout per region.

## Open Questions
- Choose final log backend (Loki vs ClickHouse) and archive strategy.
- Do we need custom dashboards in the console for databases or is Grafana embedding sufficient?
- Should we support remote write to customer-owned telemetry stacks for dedicated databases?
