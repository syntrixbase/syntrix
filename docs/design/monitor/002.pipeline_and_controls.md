...
- **Label discipline**: Enforce mandatory attributes (`service`, `database`, `region`, `version`, `instance`, `request_id`, `trace_id`) in collectors; reject or rewrite non-compliant telemetry.
...
- **Exporters**:
  ...
  - `loki` or `clickhouse` for logs with `database` as partition key.
...
- **Context propagation**: W3C Trace Context; include `baggage` for `database` and `plan_id` (queries) when safe.
- **Span attributes (must)**: `service.name`, `service.version`, `deployment.environment`, `database`, `region`, `request_id`, `trace_id`, `span.kind`, `http.method`, `http.route`, `db.system`, `db.operation`, `messaging.system` as applicable.
- **Metrics conventions**:
  ...
  - Labels kept low-cardinality: `database`, `service`, `endpoint`, `region`, `version`, `status_code_class`.
- **Logs schema** (JSON): `timestamp`, `severity`, `service`, `database`, `trace_id`, `span_id`, `request_id`, `category`, `message`, `error.class`, `error.stack` (optional), `fields` (map). Redact secrets; hash sensitive IDs when not required.
