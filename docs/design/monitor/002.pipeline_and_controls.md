...
## Why
- Turn the high-level architecture into actionable, configurable components with guardrails for multi-database operation.
- Reduce noise and cardinality risk while preserving the ability to debug database-facing incidents quickly.
...
- **Label discipline**: Enforce mandatory attributes (`service`, `database_id`, `region`, `version`, `instance`, `request_id`, `trace_id`) in collectors; reject or rewrite non-compliant telemetry.
- **Sampling**: Tail sampling based on errors and latency; head sampling only for extremely high-volume databases/endpoints.
...
- **Processors**:
  ...
  - `tail_sampling` policies:
    ...
    - Baseline sample: 10% default; per-database overrides allowed.
    - Max per-database rate guardrail (e.g., 200 spans/sec) to prevent noisy-neighbor.
...
- **Exporters**:
  ...
  - `loki` or `clickhouse` for logs with `database_id` as partition key.
...
- **Context propagation**: W3C Trace Context; include `baggage` for `database_id` and `plan_id` (queries) when safe.
- **Span attributes (must)**: `service.name`, `service.version`, `deployment.environment`, `database_id`, `region`, `request_id`, `trace_id`, `span.kind`, `http.method`, `http.route`, `db.system`, `db.operation`, `messaging.system` as applicable.
- **Metrics conventions**:
  ...
  - Labels kept low-cardinality: `database_id`, `service`, `endpoint`, `region`, `version`, `status_code_class`.
- **Logs schema** (JSON): `timestamp`, `severity`, `service`, `database_id`, `trace_id`, `span_id`, `request_id`, `category`, `message`, `error.class`, `error.stack` (optional), `fields` (map). Redact secrets; hash sensitive IDs when not required.
...
- **Templates**: include runbook link, recent deploy SHA, top databases/endpoints, exemplar trace link.

## Dashboards (How)
- **Service owner**: RED/USE, dependency health, deploy markers, error budget burn, exemplar jump-to-trace.
- **Database slice**: latency/error by database, throttling counts, sampled trace rate, log volume by database.
...
- Access control on Grafana/Tempo/Loki with database-aware RBAC where possible; audit log of queries.
...
3) Prod canary: one region with per-database rate caps; monitor collector drops; then expand.
4) Backfill docs: console integration story (embed Grafana or link-outs), database SLO exposure decision.

## Open Questions
- Finalize log backend choice (Loki vs ClickHouse) and archive strategy.
- Need per-database custom SLOs surfaced in console?
- Should we allow database-provided OTLP endpoints for dedicated tiers?
