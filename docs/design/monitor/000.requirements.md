...
- Protect availability and latency SLOs for database-facing APIs by catching regressions before users notice.
...
- Dashboards exist per service and per database slice with at-a-glance health and drill-down paths.
...
- Include `database_id`, `service`, `endpoint`, `region`, `version`, and `instance` labels; enforce low-cardinality guardrails.
...
- Add span attributes for `database_id`, `request_id`, `user_id` (when allowed), and `plan_id` for queries.
- Tail-sample high-volume surfaces with policy: default 10% for healthy traffic, 100% for errors, 100% for latency p95>target.

### Logs
- Structured JSON logs with `timestamp`, `severity`, `service`, `database_id`, `trace_id`, `span_id`, `request_id`, and message; no plain-text free-form logs.
...
- Alerts include runbook links, recent deploy info, and top correlated signals (error class, database, region).

### Dashboards and Reporting
- Service owner dashboards: golden signals, saturation, dependency health, deploy markers.
- Database views: per-database error/latency, throttling, and resource usage.
...
### Multi-database and Isolation
- Mandatory `database_id` label on metrics/logs/traces; validate in collectors to reject unlabeled data.
- Card limits per database on log volume and trace sampling to prevent noisy-neighbor impact.
...
- Sampling policies configurable per environment (dev/stage/prod) and per database tier.
...
- Do we need per-database custom SLOs exposed in the console UI?
- Should we index select span attributes (database, query plan id) for faster trace search?
