...
- Include `database`, `service`, `endpoint`, `region`, `version`, and `instance` labels; enforce low-cardinality guardrails.
...
- Add span attributes for `database`, `request_id`, `user_id` (when allowed), and `plan_id` for queries.
- Tail-sample high-volume surfaces with policy: default 10% for healthy traffic, 100% for errors, 100% for latency p95>target.

### Logs
- Structured JSON logs with `timestamp`, `severity`, `service`, `database`, `trace_id`, `span_id`, `request_id`, and message; no plain-text free-form logs.
...
- Mandatory `database` label on metrics/logs/traces; validate in collectors to reject unlabeled data.
