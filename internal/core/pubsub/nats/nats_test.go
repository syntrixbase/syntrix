...
func TestPartitioner(t *testing.T) {
	// Test partitioner type
	var p pubsub.Partitioner = func(data []byte) uint32 {
		// Simple hash
		var sum uint32
		for _, b := range data {
			sum += uint32(b)
...
func TestConsumerOptions_Partitioner(t *testing.T) {
	opts := pubsub.ConsumerOptions{
		StreamName:   "TEST",
		ConsumerName: "test-consumer",
		NumWorkers:   4,
		Partitioner: func(data []byte) uint32 {
			return 0 // Always route to worker 0
		},
	}

	assert.NotNil(t, opts.Partitioner)
	assert.Equal(t, uint32(0), opts.Partitioner([]byte("anything")))
}
...
func TestOnMessageCallback(t *testing.T) {
	...
	if opts.OnMessage != nil {
		opts.OnMessage("test.subject", nil, 50*time.Millisecond)
	}

	assert.Equal(t, "test.subject", calledSubject)
	assert.NoError(t, calledErr)
	assert.Equal(t, 50*time.Millisecond, calledLatency)
}
