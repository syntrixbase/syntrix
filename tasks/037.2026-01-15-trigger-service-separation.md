...
- [ ] Ensure all shared types are in `internal/trigger/types/`
- [ ] Move `FatalError` to types if not already there
- [ ] Verify `Trigger`, `DeliveryTask`, `RetryPolicy`, `Duration` are in types
...
- [ ] Define `Dependencies` struct
- [ ] Implement `NewService(deps, opts) (Service, error)`
- [ ] Wire CEL evaluator, watcher, and publisher
...
- [ ] Move `internal/trigger/evaluator/` contents to `internal/trigger/evaluator/cel/`
- [ ] Update imports
- [ ] Rename `celeEvaluator` to `Evaluator` (exported)

### Task 2.4: Move Watcher

- [ ] Move `internal/trigger/watcher/` to `internal/trigger/evaluator/watcher/`
- [ ] Update imports
...
- [ ] Create `TaskPublisher` struct wrapping `core/pubsub.Publisher`
- [ ] Implement subject building with encoding
- [ ] Implement subject hashing for long keys
...
- [ ] Implement `service` struct with all components
- [ ] Implement `LoadTriggers` with validation
- [ ] Implement `Start` with watch loop and evaluation
...
- [ ] Move `internal/trigger/worker/` to `internal/trigger/delivery/worker/`
- [ ] Update imports
- [ ] Export `HTTPWorker` as `Worker`

### Task 3.4: Create TaskConsumer Adapter

**File:** `internal/trigger/delivery/consumer.go`

- [ ] Create `TaskConsumer` struct wrapping `core/pubsub.Consumer`
- [ ] Implement message handler with task deserialization
- [ ] Integrate with worker for HTTP delivery
...
- [ ] Implement `service` struct with consumer
- [ ] Implement `Start` that delegates to consumer
...
- [ ] Replace `TriggerFactory` usage with separate service factories
- [ ] Add `initTriggerEvaluator(ctx)` function
- [ ] Add `initTriggerDelivery(ctx)` function
...
- [ ] Update `Start()` to start both services
- [ ] Update `Close()` to stop both services
...
- [ ] Ensure config supports both services
...
- [ ] `internal/trigger/evaluator/service_test.go` - Service lifecycle, LoadTriggers
