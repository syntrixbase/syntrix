...
|---|-------|--------|------------|
| 1 | **Design doc path mismatch** | Docs say `internal/engine/internal/{data,index,...}` but code is in `internal/query/` | **Keep `internal/query/` as package root**; use `internal/query/internal/{engine,http,client}` for unexported types. Update design docs to match. |
...
1) **Package encapsulation**
	- Move Engine/Server/Client concretes to `internal/query/internal/{engine,http,client}` with unexported types.
	- Keep `internal/query/` as package root with exported `Service` interface + factories.
...
5) **Docs update**
	- Correct design doc paths (`internal/query/internal/...` not `internal/engine/internal/...`).
