...
type TriggerEvent struct {
    ...
    TenantID    string
    Type        EventType
    Document    *storage.Document // We still use storage.Document for now as it's the common data model
...
*   `PullerEvent.Change.TenantID` -> `TriggerEvent.TenantID`
...
*   **Special Handling for Delete**:
    ...
    *   `TriggerEvent.Before` = `&storage.Document{Collection: PullerEvent.Change.MgoColl, Id: PullerEvent.Change.MgoDocID, TenantID: PullerEvent.Change.TenantID}`.
