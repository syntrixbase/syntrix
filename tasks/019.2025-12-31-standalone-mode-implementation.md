...
- [ ] **Realtime Server Watch Tenant**: `internal/api/realtime/server.go` calls `WatchCollection(ctx, "", "")` with empty tenant. Verify this works correctly with `csp.CSPService` in standalone mode. MongoDB backend must support watching all tenants when tenant is empty string.
