...
2.  **Filtering**: **Crucial**: The Watcher must filter events to only pass through those where `PullerEvent.Change.Database` matches the Watcher's configured `database`.
...
*   `PullerEvent.Change.Database` -> `TriggerEvent.Database`
...
*   **Special Handling for Delete**:
    ...
    *   `TriggerEvent.Before` = `&storage.StoredDoc{Collection: PullerEvent.Change.MgoColl, Id: PullerEvent.Change.MgoDocID, Database: PullerEvent.Change.Database}`.
