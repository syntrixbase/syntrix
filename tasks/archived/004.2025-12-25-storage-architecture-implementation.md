...
- [x] **Update Config Structure**: Modify `internal/config/config.go`.
    ...
    - Update `storage.Config` to include `Backends map[string]BackendConfig`.
    - Update `storage.Config` to include `Document`, `User`, `Revocation` fields of type `RoutingConfig`.
