...
type Service interface {
    ...
    Watch(ctx context.Context, database, collection string, resumeToken interface{}, opts storage.WatchOptions) (<-chan storage.Event, error)
}
```
...
#### Task 1.2: Implement CSPService

**File:** `internal/csp/csp_service.go` (new)
...
func (s *CSPService) Watch(ctx context.Context, database, collection string, resumeToken interface{}, opts storage.WatchOptions) (<-chan storage.Event, error) {
    // Direct call to storage.Watch() - extracted from handleWatch()
    return s.storage.Watch(ctx, database, collection, resumeToken, opts)
}
```

**Steps:**
1. Create `internal/csp/csp_service.go`
2. Implement `CSPService` struct
3. Extract watch logic from `server.go`

#### Task 1.3: Implement CSPClient

**File:** `internal/csp/csp_client.go` (new)
...
func NewCSPClient(baseURL string) *CSPClient {
    return &CSPClient{baseURL: baseURL, client: &http.Client{}}
}

func (c *CSPClient) Watch(ctx context.Context, database, collection string, resumeToken interface{}, opts storage.WatchOptions) (<-chan storage.Event, error) {
...
**Steps:**
1. Create `internal/csp/csp_client.go`
...
**Files:**
- `internal/csp/csp_service_test.go` (new)
- `internal/csp/csp_client_test.go` (new)

**Coverage targets:**
- `CSPService.Watch()` - mock storage
- `CSPClient.Watch()` - httptest server
...
```go
func (e *Engine) WatchCollection(ctx context.Context, database string, collection string) (<-chan storage.Event, error) {
...
```go
func (e *Engine) WatchCollection(ctx context.Context, database string, collection string) (<-chan storage.Event, error) {
    return e.cspService.Watch(ctx, database, collection, nil, storage.WatchOptions{})
}
```
...
// For standalone mode, use NewServiceWithCSP with an CSPService.
func NewService(store storage.DocumentStore, cspURL string) Service {
    return core.New(store, csp.NewCSPClient(cspURL))
}

// NewServiceWithCSP creates a Query Service with a custom CSP implementation.
// Use this for standalone mode with csp.NewService().
...
2. Update `NewService()` to wrap `csp.NewCSPClient()`
...
- [ ] **Realtime Server Watch Database**: `internal/api/realtime/server.go` calls `WatchCollection(ctx, "", "")` with empty database. Verify this works correctly with `csp.CSPService` in standalone mode. MongoDB backend must support watching all databases when database is empty string.
