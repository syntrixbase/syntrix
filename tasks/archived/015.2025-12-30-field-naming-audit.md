...
| - | ---- | ---- | ----- | ------- | -------- | ------ |
| 1 | `internal/storage/types/types.go` | 19 | User.DatabaseID | `json:"database_id"` | `json:"databaseId"` | API response format |
...
| 4 | `internal/identity/types/types.go` | 28 | Claims.DatabaseID | `json:"tid"` | `json:"tid"` | **Keep tid/oid (documented exception)** |
...
// User struct - Line 19
// Before:
DatabaseID string `json:"database_id" bson:"database_id"`
// After:
...
- [ ] User.DatabaseID JSON tag changed to `databaseId`
...
**JWT Token Format**: Keep `tid`/`oid` in claims to avoid breaking existing tokens and integrations. No migration to `databaseId`/`userId` is planned.
...
| ------- | ----- | -------- | ------ |
| Puller NormalizedEvent | DatabaseID | `database` | Bandwidth optimization |
...
| JWT Claims | DatabaseID/UserID | `tid`/`oid` | Backward compatibility |
