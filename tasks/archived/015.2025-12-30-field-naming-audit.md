...
| - | ---- | ---- | ----- | ------- | -------- | ------ |
| 1 | `internal/storage/types/types.go` | 19 | User.Database | `json:"database"` | `json:"database"` | API response format |
...
| 4 | `internal/identity/types/types.go` | 28 | Claims.Database | `json:"tid"` | `json:"tid"` | **Keep tid/oid (documented exception)** |
...
- [ ] User.Database JSON tag changed to `database`
...
**JWT Token Format**: Keep `tid`/`oid` in claims to avoid breaking existing tokens and integrations. No migration to `database`/`userId` is planned.
...
| ------- | ----- | -------- | ------ |
| Puller NormalizedEvent | Database | `database` | Bandwidth optimization |
...
| JWT Claims | Database/UserID | `tid`/`oid` | Backward compatibility |
