...
- Make database derive solely from `fullDocument.Database`.
...
- Validation: update normalization to source database only from `FullDocument.Database`; reject update events lacking `fullDocument` (no `UpdateLookup`).
...
- In `internal/puller/internal/normalizer/normalizer.go`: decode `raw.FullDocument` into `storage.StoredDoc`; database only from `fullDoc.Database`; error if empty/missing; require `fullDoc` for update/replace (UpdateLookup), else return error; map to new field names.
...
6) Documentation
- Add/modify design/README section: `fullDoc` must be `storage.StoredDoc` JSON shape, update requires `UpdateLookup`, database only from `fullDoc.Database`, Mongo provenance of `mgoColl/mgoDocId`.
