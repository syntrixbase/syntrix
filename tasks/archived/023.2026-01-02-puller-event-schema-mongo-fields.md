...
- In `internal/puller/internal/normalizer/normalizer.go`: decode `raw.FullDocument` into `storage.StoredDoc`; tenant only from `fullDoc.TenantID`; error if empty/missing; require `fullDoc` for update/replace (UpdateLookup), else return error; map to new field names.
...
6) Documentation
- Add/modify design/README section: `fullDoc` must be `storage.StoredDoc` JSON shape, update requires `UpdateLookup`, tenant only from `fullDoc.TenantID`, Mongo provenance of `mgoColl/mgoDocId`.
