...
- Enforce multi-database isolation end-to-end (Service, HTTP, replication, watch) with explicit database on requests and no cross-database reads/writes.
...
6) Multi-database behavior is implemented in code (database argument across Service/HTTP) but not yet captured in design docs or test matrices (edge validation, default database fallback, per-database isolation of index/change streams).
...
| 6 | **WatchCollection signature differs** | Current: `(ctx, database, collection)` vs Design: `Watch(ctx, database, collection, filters, startAfter)` | **Keep current signature** for this task. Expand in Task 015 when adding filter pushdown. |
...
- Multi-database checks: ensure database is required on public entrypoints (with temporary `DefaultDatabaseID` for dev), index/rebuild/checkpoint partitioning per database, and watch/replication routing stays database-scoped.
...
// Service interface (exported) - every method takes explicit database
type Service interface {
    GetDocument(ctx, database, fullpath) (Document, error)
    CreateDocument(ctx, database, doc, filters) error
    ReplaceDocument(ctx, database, doc, filters) error
    PatchDocument(ctx, database, doc, filters) error
    DeleteDocument(ctx, database, fullpath, filters) error
    ExecuteQuery(ctx, database, model.Query) ([]Document, error)
    Watch(ctx, database, collection, filters, startAfter) (<-chan Event, error)
    Pull/Push(ctx, database, ...) // replication
}
```
...
| Cross-database access | Reject early at handler |
...
- 2025-12-27: Multi-database support landed in code (database required on Service/HTTP/replication/watch with DefaultDatabase fallback); design docs now need alignment and tests must cover database isolation/validation.
