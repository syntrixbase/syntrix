...
- Event database source: rely on `storage.Event.Database` (and `Document.Database` when present); events lacking database are treated as invalid and dropped.
...
- Tests (testify):
  ...
  - Failure-closed behavior when `Database` missing in events (except explicit CSP/system path).
...
- Event pipeline supplies `Database`; events missing database are logged and dropped.

## Open Questions

- For local dev wildcard, should we limit to localhost/127.0.0.1 only?
- Do we need to enforce/validate that upstream watchers always populate `Database` (and fail closed if missing)? → Yes for normal paths; only privileged system/CSP watcher may omit database to see all changes (must be explicitly gated and non-default).
