...
- Introduce `TriggerFactory` + `TriggerEngine` with error-returning `LoadTriggers`, enforcing database/collection/docKey validation and fail-fast startup.
- Encapsulate watch/checkpoint logic in a `DocumentWatcher` adapter with database-scoped checkpoints, context-aware cancellation, and explicit start-from-now opt-in + audit signal.
...
- Strengthen worker HTTP path (configurable timeout/client, database-scoped secret/token retrieval, signature header) and retry/backoff parity with design.
...
- [x] **Implement Validation**:
    ...
    - Check database/collection charset (alphanumeric, dash, underscore).
...
- [x] **Update Checkpoints**:
    - Change checkpoint key to `sys/checkpoints/trigger_evaluator/<database>[/<collection>]`.
...
- [x] **Enhance Worker**:
    ...
    - [x] Implement `GenerateSystemToken` using `identity.AuthN` (already there, verify database scope).
