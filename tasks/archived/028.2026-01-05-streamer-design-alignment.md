...
- 保留 `database` 字段，移除 `document_id`（通过 Filter 表达）
...
### Target (per design doc + database)
```protobuf
message SubscribeRequest {
  string subscription_id = 1;
  string collection = 2;
  repeated Filter filters = 3;  // ✅ 结构化
  string database = 4;            // 多租户支持
  // document_id 通过 Filter(field="_id", op="==", value=...) 表达
}
...
2. 修改 SubscribeRequest：使用 `repeated Filter`，保留 `database`，移除 `document_id`
