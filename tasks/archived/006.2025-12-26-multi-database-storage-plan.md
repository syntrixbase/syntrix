...
3) Mongo implementation: add `database_id` field handling, database-aware `_id` generation (prefix format `database_id:hash(...)`), `collection_hash` population, filters, watch guards (using prefix match), and indexes.
...
- M2: Mongo documents add `database_id`, database-aware `_id`, filters, `(database_id, collection_hash)` index; EnsureIndexes updated; watch adds database guard.
...
- M4: User/Revocation models gain `database_id`, database-aware `_id`, per-database unique `(database_id, username)`; revocation per-database TTL/index; migrate call sites.
- M5: Migration/backfill guidance (dev/test data): add `database_id`/`collection_hash` and rerun EnsureIndexes.
...
- Backfill/EnsureIndexes: define handling for existing dev/test data (add `database_id`, `collection_hash`); EnsureIndexes to enforce required indexes.
