...
| 4 | `internal/identity/types/types.go` | 28 | Claims.TenantID | `json:"tid"` | `json:"tenantId"` | JWT token format |
| 5 | `internal/identity/types/types.go` | 29 | Claims.UserID | `json:"oid"` | `json:"userId"` | JWT token format |
...
**JWT Token Format Change**: Changing `tid`/`oid` to `tenantId`/`userId` will invalidate existing tokens. Plan:

1. Add new fields alongside old ones during transition
2. Accept both formats during token validation
3. Remove old fields after token expiry period

### Identity Acceptance Criteria

- [ ] Claims struct uses standard field names
